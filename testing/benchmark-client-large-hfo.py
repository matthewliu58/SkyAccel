...
SERVER_PORT = 8081
CONCURRENCY = 400
NUM_CYCLES = 5  # Number of burst-pause cycles
BURST_DURATION = 20  # Duration of burst test in seconds
PAUSE_DURATION = 20  # Duration of pause in seconds
SOCK_TIMEOUT = 10
...
if __name__ == "__main__":
    ...
    print(f"Cycles: {NUM_CYCLES}, Burst: {BURST_DURATION}s, Pause: {PAUSE_DURATION}s")
    print(f"Total duration: {NUM_CYCLES} cycles x {BURST_DURATION + PAUSE_DURATION}s = {NUM_CYCLES * (BURST_DURATION + PAUSE_DURATION)}s")
    
    # Burst control event
    burst_event = threading.Event()
    
    # Start all client workers (they will wait for burst_event)
    for idx in range(CONCURRENCY):
        t = threading.Thread(target=client_worker, args=(burst_event,), name=f"CLIENT-{idx+1}", daemon=True)
        t.start()
    
    # Main control loop: alternate between burst and pause
    for cycle in range(1, NUM_CYCLES + 1):
        print(f"\n=== Cycle {cycle}/{NUM_CYCLES}: BURST ({BURST_DURATION}s) ===")
        
        # Start burst
        burst_event.set()
        
        # Run burst for BURST_DURATION seconds
        time.sleep(BURST_DURATION)
        
        # Stop burst (workers will wait)
        burst_event.clear()
        
        if cycle < NUM_CYCLES:
            print(f"=== Cycle {cycle}: PAUSE ({PAUSE_DURATION}s) ===")
            # Pause for PAUSE_DURATION seconds
            time.sleep(PAUSE_DURATION)
    
    stop_event.set()
    burst_event.set()  # Ensure any waiting workers can exit
