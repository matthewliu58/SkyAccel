...
routing_table = []  # List of {server_ip, rtt}
            ...
            if resp.status_code == 200:
                ...
                if data.get("code") == 200 and "data" in data:
                    ...
                    if "routing" in routing_info:
                        routes = routing_info["routing"]
                        
                        # Extract server IPs and RTTs
                        table = []
                        for route in routes:
                            rtt = route.get("rtt", 0)
                            if rtt <= 0:
                        ...
                        if table:
                            ...
                            print(f"[ROUTING] Updated {len(routing_table)} routes")
        except Exception as e:
            print(f"[ROUTING] Fetch failed: {e}")
        
        time.sleep(1)

def select_server_by_p2c():
    """Select server IP using Power of Two Choices (P2C) algorithm"""
    with routing_lock:
        ...
        if len(routing_table) == 1:
            return routing_table[0]["server_ip"]
        
        # Randomly select two different servers
        idx1, idx2 = random.sample(range(len(routing_table)), 2)
        
        # Compare RTT and select the one with smaller RTT
        if routing_table[idx1]["rtt"] <= routing_table[idx2]["rtt"]:
            return routing_table[idx1]["server_ip"]
        else:
            return routing_table[idx2]["server_ip"]
...
def client_worker():
    ...
    while not stop_event.is_set():
        # Select server using P2C algorithm
        server_ip = select_server_by_p2c()
        if not server_ip:
...
if __name__ == "__main__":
    ...
    print(f"Selection algorithm: Power of Two Choices (P2C)")
